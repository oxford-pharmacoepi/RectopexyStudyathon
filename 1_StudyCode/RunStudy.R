...
for(i in seq_along(cohort_set(cdm$study_cohorts_rp)$cohort_definition_id)){
  print(paste0('-- for ', i))
  ...
  write_csv(rt_inc_rp,
            here("results", paste0(
              "rectopexy_incidence_rectal_prolapse_",i, cdmName(cdm), ".csv"
            )))
  write_csv(incidenceAttrition(rt_inc_rp),
            here("results", paste0(
              "rectopexy_incidence_attrition_rectal_prolapse_",i, cdmName(cdm), ".csv"
            )))
